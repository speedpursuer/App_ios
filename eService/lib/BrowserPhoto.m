...
@implementation BrowserPhoto
@synthesize underlyingImage = _underlyingImage; // synth property from protocol

+ (BrowserPhoto *)photoWithImage:(UIImage *)image {
	return [[BrowserPhoto alloc] initWithImage:image];
}

+ (BrowserPhoto *)photoWithURL:(NSURL *)url {
	return [[BrowserPhoto alloc] initWithURL:url];
}

+ (BrowserPhoto *)photoWithURL:(NSURL *)url size:(CGSize)size index:(NSInteger)index {
	return [[BrowserPhoto alloc] initWithURL:url size:size index:index];
}

+ (BrowserPhoto *)photoWithAsset:(PHAsset *)asset targetSize:(CGSize)targetSize {
	return [[BrowserPhoto alloc] initWithAsset:asset targetSize:targetSize];
}

+ (BrowserPhoto *)videoWithURL:(NSURL *)url {
	return [[BrowserPhoto alloc] initWithVideoURL:url];
}
...
- (id)initWithURL:(NSURL *)url size:(CGSize)size index:(NSInteger)index{
	if ((self = [super init])) {
		self.photoURL = url;
		self.size = size;
		self.index = index;
...
- (void)_performLoadUnderlyingImageAndNotifyWithWebURL:(NSURL *)url {
	@try {		
		[[CacheManager sharedManager] getImageWithKey:[url absoluteString] progress:^(NSInteger receivedSize, NSInteger expectedSize) {
			if (expectedSize > 0) {
...
//	}
	if(_loadingInProgress) {
		[[CacheManager sharedManager] cancelAllRequest];
		_loadingInProgress = NO;
	}
